...
$('li[name="navItems"]').click(function(){
	var projectNav = document.getElementById('projectDropdown');
	projectNav.className=""
});

$("#projectDropdown").click(function() {
	var listItems = document.getElementsByName('navItems');
	for(i=0; i<listItems.length; i++ ) 
	{
	   if (listItems[i].className.match(/(?:^|\s)active(?!\S)/))
	   {
		   listItems[i].className = listItems[i].className.replace(/(?:^|\s)active(?!\S)/g , '' );
	   }
	}
...
$("#projectDropdown").dblclick(function() {
    ...
    if (projects == null)
    {
    	var newurl = window.location.protocol + '#projects';
    	window.location.href = newurl;
   	}
    else
    {
	   $('html, body').stop().animate({scrollTop: $('#projects').offset().top}, 1500, 'easeInOutExpo');
       event.preventDefault();
    }
